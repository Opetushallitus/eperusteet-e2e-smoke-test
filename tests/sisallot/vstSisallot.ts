...
function vstPaikallisetOpsTekstit(opsNimi: string) {
  return [
    opsNimi,
    'Aktiivinen kansalaisuus',
    ...vstJotpaPaikallisetOpsTekstit(opsNimi),
  ];
}

function vstJotpaPaikallisetOpsTekstit(opsNimi: string) {
  return [
    opsNimi,
...
export async function jotpaOpetussuunnitelmaJulkinenTarkistukset(testData: TestData) {
  ...
  await tarkistaPdfSisalto(page.getByRole('link', { name: OPS_PDF_LINKKI }), vstJotpaPaikallisetOpsTekstit(opsNimi!));
