...
test.describe('Uusi peruste ja perusteesta OPS', async () => {
  ...
  [
    {
      ...
      sisallot: async (page: Page, url: string) => await varhaiskasvatusSisallot(page),
    },
    {
      koulutustyyppi: 'Perusopetus',
      perusteDiaari: `${Math.floor(100 + Math.random() * 900)}/${Math.floor(100 + Math.random() * 900)}/${Math.floor(1000 + Math.random() * 9000)}`,
      projektiNimi: createNimi('TestAutomation Perusopetus'),
      sisallot: async (page: Page, url: string) => await perusopetuksenSisallot(page, url),
    },
  ].forEach(({ koulutustyyppi, projektiNimi, perusteDiaari, sisallot }) => {
    perusteProjektit[koulutustyyppi] = {
      nimi: projektiNimi + ' (' + perusteDiaari + ')',
      diaari: perusteDiaari,
    };

    test(`Luo, päivitä ja julkaise peruste ${koulutustyyppi}`, async ({ page }) => {
      ...
      await sisallot(page, perusteProjektiUrl);
  ...
  ].forEach(({ koulutustyyppi, pohjaNimi, opsNimi, opsLuonti, opsSisallot, julkinenOpsTarkistukset }) => {
    test(`Luo, päivitä ja julkaise opetussuunnitelma ${koulutustyyppi}`, async ({ page }) => {
