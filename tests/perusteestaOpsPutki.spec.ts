...
import { lukioJulkinenOpsTarkistukset, lukioOpsLuonti, lukioOpsSisallot, lukioSisallot } from './lukioSisallot';
...
test.describe('Uusi peruste ja perusteesta OPS', async () => {
  ...
  [
  ...
  ].forEach(({ koulutustyyppi, projektiNimi, perusteDiaari, perusteSisallot, pohjaNimi, opsNimi, opsLuonti, opsSisallot, julkinenOpsTarkistukset }) => {
    ...
    test(`Luo, päivitä ja julkaise peruste ja ops ${koulutustyyppi}`, async ({ page }) => {
      await perusteenTestit(page, koulutustyyppi, projektiNimi, perusteDiaari, perusteSisallot);
      await opetussuunnitelmanTestit(page, koulutustyyppi, pohjaNimi, opsNimi, opsLuonti, opsSisallot, julkinenOpsTarkistukset);
    });
  });

  async function perusteenTestit(page: Page, koulutustyyppi: string, projektiNimi: string, perusteDiaari: string, perusteSisallot: (page: Page, url: string) => Promise<void>) {
